...
## Using the StateMachine Activity Designer  
 To add a \<xref:System.Activities.Statements.StateMachine> activity, drag the **StateMachine** activity designer from the **State Machine** section of the **Toolbox** and drop it on to the [!INCLUDE[wfd1](../WF_Design/includes/wfd1_md.md)] surface. To add a child state to this \<xref:System.Activities.Statements.StateMachine> activity, drag a \<xref:System.Activities.Statements.State> or \<xref:System.Activities.Core.Presentation.FinalState> from the **Toolbox** and drop it onto the **StateMachine**.  
