...
---
# Exporting Settings
The [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] integrated development environment (IDE) uses classes that implement the \<xref:Microsoft.VisualStudio.Shell.IProfileManager> interface and classes that are registered as supporting a given VSPackage implementation to save the state of a VSPackage.  
...
3.  Ensure that the class that implements the settings also persists the state to disk.  
    ...
    -   In addition to data explicitly written and transparent to the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings*> method implementation, the settings API also saves [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] version information. Therefore, saved settings can be compared against the version of the IDE that generated them during settings importation.  
