...
You are attempting to open a project created in an earlier version of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. The project must be updated to the current format, but it cannot be updated because either the specified project file (.vbproj) is marked as read-only or the file is under source control and is currently locked by another user.  
