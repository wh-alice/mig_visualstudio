...
An error or warning message is displayed if a tool returns an error or a warning while processing a .resx file. As part of the build process, the project system transforms each .resx file into a binary file understood by the [!INCLUDE[dnprdnshort](../VS_debugger/includes/dnprdnshort_md.md)] resource manager. This transformation is done using in-process tools.  
