...
This walkthrough illustrates how to use automation to create a managed VSPackage that manipulates the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] integrated development environment (IDE). You create a sample managed VSPackage and then use automation methods in the resulting VSPackage to display [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] properties in the **Output** window.  
...
1.  Create a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Package project named `Auto`.  
...
2.  On the **Select a Programming Language** page, set the language to [!INCLUDE[csprcs](../VS_debugger/includes/csprcs_md.md)].  
...
3.  In the AutoPackage file, add the following namespace reference.  
     ...
     [!code[VSSDKAuto#1](../VS_not_in_toc/codesnippet/CSharp/walkthrough--extending-managed-vspackages-by-using-automation_1.cs)]
[!code[VSSDKAuto#1](../VS_not_in_toc/codesnippet/VisualBasic/walkthrough--extending-managed-vspackages-by-using-automation_1.vb)]  
  
4.  In the AutoPackage file, replace the body of the `MenuItemCallback` method with the following lines:  
  
     [!code[VSSDKAuto#2](../VS_not_in_toc/codesnippet/CSharp/walkthrough--extending-managed-vspackages-by-using-automation_2.cs)]
[!code[VSSDKAuto#2](../VS_not_in_toc/codesnippet/VisualBasic/walkthrough--extending-managed-vspackages-by-using-automation_2.vb)]  
  
 This code calls \<xref:Microsoft.VisualStudio.Shell.Package.GetService*> to obtain a \<xref:EnvDTE.DTE> automation object that represents the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] IDE. The automation code in `MenuItemCallback` creates a new pane in the **Output** window named **Test**. The [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] name and version is then written to the new **Output** pane.  
  
1.  Build and start the Auto project in debug mode by pressing F5.  
  
     This starts the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] experimental build ([!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Exp).  
  
    > [!NOTE]
    >  Both versions of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] are open at this point.  
  
2.  In [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Exp, on the **Tools** menu, click **Auto**.  
    ...
    ```  
     ...
     Where x.xx is the latest [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] version number.  
