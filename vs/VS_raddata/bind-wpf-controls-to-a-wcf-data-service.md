...
In this walkthrough, you will create a WPF application that contains data-bound controls. The controls are bound to customer records that are encapsulated in a [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)]. You will also add buttons that customers can use to view and update records.  
  
 This walkthrough illustrates the following tasks:  
  
-   Creating an Entity Data Model that is generated from data in the AdventureWorksLT sample database.  
  
-   Creating a [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)] that exposes the data in the Entity Data Model to a WPF application.  
...
-   Creating a button that saves changes to data in the controls to the [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)] and the underlying data source.  
...
-   [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]  
...
-   Data models in [!INCLUDE[ssAstoria](../VS_raddata/includes/ssastoria_md.md)].  
...
## Create the service project  
 Start this walkthrough by creating a project for a [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)].  
...
## Create an Entity Data Model for the service  
 To expose data to an application by using a [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)], you must define a data model for the service. The [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)] supports two types of data models: Entity Data Models, and custom data models that are defined by using common language runtime (CLR) objects that implement the <xref:System.Linq.IQueryable`1> interface. In this walkthrough, you create an Entity Data Model for the data model.  
...
## Create the service  
 Create a [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)] to expose the data in the Entity Data Model to a WPF application.  
...
1.  In the `AdventureWorks.svc` code file, replace the `AdventureWorksService` class declaration with the following code.  
     ...
     [!code[Data_WPFWCF#1](../VS_raddata/codesnippet/CSharp/bind-wpf-controls-to-a-wcf-data-service_1.cs)]
[!code[Data_WPFWCF#1](../VS_raddata/codesnippet/VisualBasic/bind-wpf-controls-to-a-wcf-data-service_1.vb)]  
...
## Create the WPF client application  
 To display the data from the [!INCLUDE[ss_data_service](../VS_raddata/includes/ss_data_service_md.md)], create a new WPF application with a data source that is based on the service. Later in this walkthrough, you will add data-bound controls to the application.  
...
2.  In the [!INCLUDE[TLA#tla_titlexaml](../VS_raddata/includes/tlasharptla_titlexaml_md.md)] view of the designer, add the following code between the `<Grid>` tags:  
...
2.  Replace the event handler with the following code. Make sure that you replace the *localhost* address in this code with the local host address on your development computer.  
     ...
     [!code[Data_WPFWCF#2](../VS_raddata/codesnippet/CSharp/bind-wpf-controls-to-a-wcf-data-service_2.cs)]
[!code[Data_WPFWCF#2](../VS_raddata/codesnippet/VisualBasic/bind-wpf-controls-to-a-wcf-data-service_2.vb)]  
...
2.  Add the following code to the generated `backButton_Click` event handler:  
     ...
     [!code[Data_WPFWCF#3](../VS_raddata/codesnippet/CSharp/bind-wpf-controls-to-a-wcf-data-service_3.cs)]
[!code[Data_WPFWCF#3](../VS_raddata/codesnippet/VisualBasic/bind-wpf-controls-to-a-wcf-data-service_3.vb)]  
...
4.  Add the following code to the generated `nextButton_Click` event handler:  
     ...
     [!code[Data_WPFWCF#4](../VS_raddata/codesnippet/CSharp/bind-wpf-controls-to-a-wcf-data-service_4.cs)]
[!code[Data_WPFWCF#4](../VS_raddata/codesnippet/VisualBasic/bind-wpf-controls-to-a-wcf-data-service_4.vb)]  
...
2.  Add the following code to the `saveButton_Click` event handler.  
     ...
     [!code[Data_WPFWCF#5](../VS_raddata/codesnippet/CSharp/bind-wpf-controls-to-a-wcf-data-service_5.cs)]
[!code[Data_WPFWCF#5](../VS_raddata/codesnippet/VisualBasic/bind-wpf-controls-to-a-wcf-data-service_5.vb)]  
