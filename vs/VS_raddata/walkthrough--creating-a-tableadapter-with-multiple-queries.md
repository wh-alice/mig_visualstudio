...
1.  In [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], from the **File** menu, create a new project.  
...
4.  Replace the code in the `Form1_Load` event handler with the following to run the `FillByCity` query.  
     ...
     [!code[VbRaddataTableAdapters#1](../VS_raddata/codesnippet/VisualBasic/walkthrough--creating-a-tableadapter-with-multiple-queries_1.vb)]
[!code[VbRaddataTableAdapters#1](../VS_raddata/codesnippet/CSharp/walkthrough--creating-a-tableadapter-with-multiple-queries_1.cs)]  
...
-   Add validation logic to the \<xref:System.Data.DataTable.ColumnChanging> or \<xref:System.Data.DataTable.RowChanging> event of the data tables in the dataset. For more information, see [Validate data in datasets](../VS_raddata/validate-data-in-datasets.md).  
