...
This walkthrough demonstrates how to use [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Graphics Diagnostics to investigate an object that is colored incorrectly due to a shader bug.  
...
1.  In [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], load a graphics log that contains a frame that exhibits the missing model. A new graphics log document window appears in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. In the top part of this window is the render target output of the selected frame. In the bottom part is the **Frame List**, which displays each captured frame as a thumbnail image.  
