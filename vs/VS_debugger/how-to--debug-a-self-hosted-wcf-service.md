...
A *self-hosted service* is a WCF service that does not run inside IIS, the WCF Service Host, or the [!INCLUDE[vstecasp](../dv_TeamTestALM/includes/vstecasp_md.md)] Development Server. The easiest way to debug a self-hosted WCF is to configure [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] to launch both client and server when you choose **Start Debugging** on the **Debug** menu.  
...
1.  Create a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution that contains both the client and server projects.  
