...
## Using Graphics Diagnostics to debug rendering problems  
 ...
 The Graphics Diagnostics tools in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] are designed to help you locate rendering problems by starting with the visual artifacts that indicate the problem and then tracing back to the source of the problem by focusing only on relevant shader code, pipeline stages, draw calls, resources, and device state—in the app's own source code.  
...
### Windows 8.1 and Direct3D 11.2  
 In [!INCLUDE[win81](../VS_debugger/includes/win81_md.md)], DirectX 11.2 introduces new features that include support for capturing graphics information through its runtime. [!INCLUDE[win81](../VS_debugger/includes/win81_md.md)] uses the new runtime-based capture—known as *robust capture*—exclusively for all versions of DirectX that [!INCLUDE[win81](../VS_debugger/includes/win81_md.md)] supports. Robust capture also supports new features of Direct3D 11.2.  
