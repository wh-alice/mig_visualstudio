...
You can use [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Graphics Diagnostics to programmatically capture graphics information from a Direct3D app.  
...
> [!NOTE]
>  Previous implementations of programmatic capture relied on Remote Tools for [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] to provide capture functionality, Windows 8.1 supports capture directly through Direct3D 11.2. As a result, you no longer have to install the Remote Tools for programmatic capture on Windows 8.1.  
...
> [!IMPORTANT]
>  When using programmatic capture, you must still run your app under graphics diagnostics (Alt+F5 in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]) or under the [Command-Line Capture Tool](../VS_debugger/command-line-capture-tool.md).  
...
-   Use the following code to hook up the IDXGraphicsAnalysis interface to the DXGI debug interface.  
    ...
    > [!NOTE]
    >  If `DXGIGetDebugInterface1` returns `E_NOINTERFACE` (`error: E_NOINTERFACE No such interface supported`), make sure the app is running under graphics diagnostics (Alt+F5 in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]).  
...
### Preparing your computer to use programmatic capture  
 The programmatic capture API uses Remote Tools for [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] to provide capture functionality. The computer where the app will run must have the remote tools installed, even when you are using programmatic capture on your local computer. [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] doesn't have to be running when you perform programmatic capture on a local computer.  
  
 To use the remote capture APIs in an app that's running on a computer, you first have to install the Remote Tools for [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] on that computer. Different versions of the remote tools support different hardware platforms. For information about how to install the remote tools, see the [Remote Tools download page](http://go.microsoft.com/fwlink/p/?LinkId=246691) on the Microsoft downloads website.  
  
 Alternatively, [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] installs the necessary components to perform remote capture for 32-bit apps.  
  
> [!NOTE]
>  Because most Windows desktop apps—including [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]—are not supported on [!INCLUDE[win8](../dv_TeamTestALM/includes/win8_md.md)] for ARM devices, using Remote Tools for [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] together with the programmatic capture API is the only way to capture graphics diagnostics on ARM devices.  
