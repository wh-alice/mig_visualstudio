...
-   \<xref:System.Diagnostics.Debug.Assert*?displayProperty=fullName> and \<xref:System.Diagnostics.Trace.Assert*?displayProperty=fullName> methods, which break execution and outputs information if a specified condition fails. By default, the `Assert` method displays the information in a dialog box. For more information, see [Assertions in Managed Code](../VS_debugger/assertions-in-managed-code.md).  
