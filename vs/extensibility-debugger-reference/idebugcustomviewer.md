---
title: "IDebugCustomViewer | testtitle"
ms.custom: ""
ms.date: "10/19/2016"
...
## Remarks  
 ...
 A custom viewer is registered in the same way as an EE and, therefore, requires a language GUID and a vendor GUID. The exact metric (or registry entry name) is known only to the EE. This metric is returned in the [DEBUG_CUSTOM_VIEWER](../extensibility-debugger-reference/debug_custom_viewer.md) structure, which in turn is returned by a call to [GetCustomViewerList](../extensibility-debugger-reference/idebugproperty3--getcustomviewerlist.md). The value stored in the metric is the `CLSID` that is passed to COM's `CoCreateInstance` function (see the Example).  
