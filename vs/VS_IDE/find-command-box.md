...
You can search for text and run Visual Studio commands from the **Find/Command** box. The **Find/Command** box is still available as a toolbar control, but is no longer visible by default. You can display the **Find/Command** box by choosing **Add or Remove Buttons** on the **Standard** toolbar and then choosing **Find**.  
 ...
 To run a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] command, preface it with a greater than (>) sign.  
...
## Entering Commands  
 To use the **Find/Command** box to issue a single [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] command or alias rather than search for text, enter the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] command, prefaced with a greater than (>) symbol. For example:  
