...
## Error Reporting and the Output Directive  
 Any errors that arise during processing will be displayed in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] error window. In addition, you can be notified of errors by specifying a callback that implements \<xref:Microsoft.VisualStudio.TextTemplating.VSHost.ITextTemplatingCallback>.  
...
```  
 ...
 The compiler warning will appear in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] error window, and it will also generate a call to `ErrorCallback`.  
...
## Related Topics  
 ...
 To generate text outside a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extension:  
