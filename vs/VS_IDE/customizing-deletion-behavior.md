...
-   [Deletion Events](#rules) – Use store events to propagate updates outside the store, for example to other [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] documents.  
...
2.  **Deleted** store event is invoked at the end of a transaction, and is called after an undo or redo. It can therefore be used to propagate deletions to objects outside the store such as files, database entries or other objects in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
