...
## Rule Description  
 When the name of an instance field matches a parameter or a local variable name, the instance field is accessed by using the `this` (`Me` in [!INCLUDE[vbprvb](../VS_debugger/includes/vbprvb_md.md)]) keyword when inside the method body. When maintaining code, it is easy to forget this difference and assume that the parameter/local variable refers to the instance field, which leads to errors. This is true especially for lengthy method bodies.  
...
## Example  
 ...
 [!code[FxCop.Maintainability.VarMatchesField#1](../VS_IDE/codesnippet/VisualBasic/ca1500--variable-names-should-not-match-field-names_1.vb)]
[!code[FxCop.Maintainability.VarMatchesField#1](../VS_IDE/codesnippet/CSharp/ca1500--variable-names-should-not-match-field-names_1.cs)]