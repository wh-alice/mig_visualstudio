...
[!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] ModelBus provides a method for creating links between models and from other tools into models. For example, you could link domain-specific language (DSL) models and UML models. You can create an integrated set of DSLs.  
...
> [!NOTE]
>  In the current [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] implementation of ModelBus, the linked models must be items in the same [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution.  
...
##  <a name="provide"></a> Providing Access to a DSL  
 Before you can create ModelBus references to a model or its elements, you must define a ModelBusAdapter for the DSL. The easiest way to do this is to use the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Model Bus Extension, which adds commands to the DSL Designer.  
...
### Making sure that elements can be referenced  
 [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] ModelBus adapters use the guid of an element to identify it, by default. These identifiers must therefore be persisted in the model file.  
...
3.  In the Debugging project in the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], add files that are instances of each DSL.  
  
    > [!NOTE]
    >  [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] ModelBus can only resolve references to models that are items in the same [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution. For example, you cannot create a reference to a model file in another part of your file system.  
...
## Creating References in Program Code  
 ...
 To create a model reference, you need the AdapterManager of the DSL of which the model is an instance, and the filename or [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] project item of the model.  
  
 To create an element reference, you need an adapter for the model file, and the element you want to refer to.  
  
> [!NOTE]
>  With the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] ModelBus, you can create references only to items in the same [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution.  
