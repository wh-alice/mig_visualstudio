...
## What can you do with a Domain-Specific Language?  
 ...
 When you have designed a DSL, you can distribute it as part of a Visual Studio Integration Extension (VSIX) package. Users work with the DSL in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]:  
 ...
 The rest of this overview is a walkthrough that introduces the basic operations of creating and using a domain-specific language in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
...
|[!INCLUDE[vssdk_current_short](../VS_IDE/includes/vssdk_current_short_md.md)]|[http://go.microsoft.com/fwlink/?LinkId=185580](http://go.microsoft.com/fwlink/?LinkId=185580)|  
...
## Creating a DSL Solution  
 To create a new domain-specific language, you create a new [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution by using the Domain-Specific Language project template.  
...
-   **DslPackage project** This project contains code that allows instances of the DSL to be opened and edited in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
...
2.  Press F5, or on the **Debug** menu, click **Start Debugging**.  
     ...
     The DSL builds and is installed in the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
  
     An experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] starts. The experimental instance takes its settings from a separate subtree of the registry, where [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extensions are registered for debugging purposes. Normal instances of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] do not have access to extensions registered there.  
  
3.  In the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], open the model file named **Test** from **Solution Explorer**.  
...
5.  Click the labels of the shapes to change them.  
 ...
 Your experimental [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] will resemble the following example:  
...
### The Content of a Model  
 ...
 If you cannot see the Explorer in the debugging instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], on the **View** menu point to **Other Windows**, and then click *\<Your Language>* **Explorer**.  
...
1.  In the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], save the model file.  
...
## Customizing the DSL  
 When you want to modify the DSL definition, close the experimental instance and update the definition in the main [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] instance.  
...
3.  Rebuild the DSL.  
    ...
    3.  Press F5. Wait until the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] appears.  
  
4.  In the Debugging solution in the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], open a test model file. Drag elements onto it from the toolbox. Notice that the tool captions and the type names in DSL Explorer have changed.  
...
9. In the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], open a test model file. Use the new tools to create towns and links between towns and persons. Notice that you can only create links between the correct types of element.  
...
## Validation and Commands  
 ...
 You can also define menu commands that the user can invoke. Commands can modify the model. They can also interact with other models in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] and with external resources. For more information, see [How to: Modify a Standard Menu Command](../VS_IDE/how-to--modify-a-standard-menu-command-in-a-domain-specific-language.md).  
  
## Deploying the DSL  
 To allow other users to use the domain-specific language, you distribute a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Extension (VSIX) file. This is created when you build the DSL solution.  
  
 Locate the .vsix file in the bin folder of your solution. Copy it to the computer on which you want to install it. On that computer, double-click the VSIX file. The DSL can be used in all instances of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] on that computer.  
  
 You can use the same procedure to install the DSL on your own computer so that you do not have to use the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
...
##  <a name="Reset"></a> Removing old Experimental DSLs  
 If you have created experimental DSLs that you no longer want, you can remove them from your computer by resetting the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Experimental instance.  
  
 This will remove from your computer all experimental DSLs and other experimental [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extensions. These are extensions that have been executed in debugging mode.  
  
 This procedure does not remove DSLs or other [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extensions that have been fully installed by executing the VSIX file.  
...
2.  Rebuild any experimental DSLs or other experimental [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extensions that you still want to use.  
