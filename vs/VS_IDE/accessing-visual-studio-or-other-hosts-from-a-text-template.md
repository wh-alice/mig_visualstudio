...
In a text template, you can use methods and properties exposed by the host that executes the template, such as [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
...
### Displaying Error Messages  
 This example logs messages when you transform the template. If the host is [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], they are added to the error window.  
...
## Using the Visual Studio API  
 If you are executing a text template in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], you can use `this.Host` to access services provided by [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] and any packages or extensions that are loaded.  
  
 Set hostspecific="true" and cast `this.Host` to \<xref:System.IServiceProvider>.  
  
 This example gets the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] API, \<xref:EnvDTE.DTE>, as a service:  
