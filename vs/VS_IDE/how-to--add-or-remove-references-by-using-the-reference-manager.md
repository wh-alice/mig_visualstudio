...
## Assemblies tab  
 ...
 All desktop projects contain an implicit reference to mscorlib. [!INCLUDE[vbprvb](../VS_debugger/includes/vbprvb_md.md)] projects contain an implicit reference to Microsoft.VisualBasic. In [!INCLUDE[vs_dev11_long](../dv_TeamTestALM/includes/vs_dev11_long_md.md)], all projects contain an implicit reference to System.Core, even if it’s removed from the list of references.  
...
2.  Extensions lists all assemblies that external vendors of components and controls have developed to extend the targeted Framework. Depending on the purpose of the user application, it might need these assemblies.  
 ...
 Some components in the list may not be shown, depending on the [!INCLUDE[dnprdnshort](../VS_debugger/includes/dnprdnshort_md.md)] version of your project. This can occur under the following conditions:  
...
-   A component that uses [!INCLUDE[net_v40_short](../VS_debugger/includes/net_v40_short_md.md)] is incompatible with a project that targets the [!INCLUDE[net_v45](../VS_IDE/includes/net_v45_md.md)].  
  
     When you create a new application, some projects target the [!INCLUDE[net_v45](../VS_IDE/includes/net_v45_md.md)] by default. For more information, see [.NET Framework Client Profile](../Topic/.NET%20Framework%20Client%20Profile.md).  
...
## Solution tab  
 ...
 A project can reference another project that targets a different version of the .NET Framework. For example, you could create a project that targets the [!INCLUDE[net_v40_short](../VS_debugger/includes/net_v40_short_md.md)] but that references an assembly that’s been built for the .NET Framework 2. However, the .NET Framework 2 project can’t reference a [!INCLUDE[net_v40_short](../VS_debugger/includes/net_v40_short_md.md)] project. For more information, see [Targeting a Specific .NET Framework Version](../VS_IDE/targeting-a-specific-.net-framework-version.md).  
  
 A project that targets the [!INCLUDE[net_v40_short](../VS_debugger/includes/net_v40_short_md.md)] is incompatible with a project that targets the [!INCLUDE[net_client_v40_long](../VS_IDE/includes/net_client_v40_long_md.md)].  
  
 In [!INCLUDE[vs_dev11_long](../dv_TeamTestALM/includes/vs_dev11_long_md.md)], a file reference instead of a project reference is created if one project targets the .NET Framework 4 and another project targets an earlier version.  
  
 A project that targets the [!INCLUDE[net_win8_profile](../VS_IDE/includes/net_win8_profile_md.md)] can’t add a project reference to a project that targets the .NET Framework and vice versa.  
...
### Core Subgroup  
 ...
 [!INCLUDE[win8_appname_long](../VS_debugger/includes/win8_appname_long_md.md)] app projects contain references to all of the WinMDs in the [!INCLUDE[win8](../dv_TeamTestALM/includes/win8_md.md)] SDK by default on project creation. In managed projects, a read-only node under the References folder in **Solution Explorer** indicates the reference to the entire [!INCLUDE[win8](../dv_TeamTestALM/includes/win8_md.md)] SDK. Accordingly, the Core subgroup in the Reference Manager won’t enumerate any of the assemblies from the [!INCLUDE[win8](../dv_TeamTestALM/includes/win8_md.md)] SDK and instead displays a message: “The Windows SDK is already referenced. Please use the Object Browser to explore the references in the Windows SDK.”  
...
### Extensions Subgroup  
 Extensions lists the user SDKs that extend the targeted Windows platform. This tab appears for [!INCLUDE[win8_appname_long](../VS_debugger/includes/win8_appname_long_md.md)] app projects only. Desktop projects won’t show this tab because they can consume only first-party .winmd files.  
