...
|`Exclude`|Optional \<xref:Microsoft.Build.Framework.ITaskItem>`[]` output parameter.<br /><br /> Specifies the items to exclude from the output item collection. This parameter can contain wildcard specifications. For more information, see [Items](../VS_IDE/msbuild-items.md) and [How to: Exclude Files from the Build](../VS_IDE/how-to--exclude-files-from-the-build.md).|  
