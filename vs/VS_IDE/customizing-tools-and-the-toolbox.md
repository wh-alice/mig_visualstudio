...
5.  To test the DSL, press F5 or CTRL+F5, and in the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], open a sample model file. The new tool should appear on the toolbox. Drag it onto the diagram to verify that it creates a new element.  
  
     If the tool does not appear, stop the experimental [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. In the Windows **Start** menu, run **Reset the Microsoft Visual Studio 2010 Experimental Instance**. On the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]**Build** menu, click **Rebuild Solution**. Then test the DSL again.  
