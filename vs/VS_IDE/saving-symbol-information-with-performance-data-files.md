...
-   [!INCLUDE[vsUltLong](../VS_IDE/includes/vsultlong_md.md)], [!INCLUDE[vsPreLong](../VS_IDE/includes/vsprelong_md.md)], [!INCLUDE[vsPro](../VS_IDE/includes/vspro_md.md)]  
 ...
 You can serialize symbols from the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] IDE or from the command-line:  
  
-   To serialize symbols in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] IDE, point to **Tools** on the menu bar, and then click **Options**. In the **Options** window, select **Performance Tools**, and then select the **Automatically serialize symbol information** check box.  
...
-   Make sure that you run vsperfreport with the /PACKSYMBOLS flag or, in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] IDE, that you have the serialize symbol information option selected in the general performance explorer options.  
...
-   Make sure that you have set the path of your Windows symbol cache. Do one of the following to set the symbol cache path:  
    ...
    -   Set the Debugger->Symbols option in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] IDE to the correct path.  
  
    -   Add the -symbolpath option to the VSPerfReport command line to include your symbols.  
  
-   If you do not see any symbols in [!INCLUDE[vstecasp](../dv_TeamTestALM/includes/vstecasp_md.md)], make sure that you have the symbol server set up correctly for the ASP server.  
