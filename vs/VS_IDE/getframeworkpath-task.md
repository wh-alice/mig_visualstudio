...
## Remarks  
 If several versions of the [!INCLUDE[dnprdnshort](../VS_debugger/includes/dnprdnshort_md.md)] are installed, this task returns the version that [!INCLUDE[vstecmsbuild](../VS_IDE/includes/vstecmsbuild_md.md)] is designed to run on.  
...
## Example  
 The following example uses the `GetFrameworkPath` task to store the path to the [!INCLUDE[dnprdnshort](../VS_debugger/includes/dnprdnshort_md.md)] in the `FrameworkPath` property.  
