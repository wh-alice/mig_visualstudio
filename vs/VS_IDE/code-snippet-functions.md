...
There are three functions available to use with [!INCLUDE[csprcs](../VS_debugger/includes/csprcs_md.md)] code snippets. Functions are specified in the [Function](assetId:///572c5549-5821-4e15-8ecd-0fa86c1c65df) element of the code snippet. For information on creating code snippets, see [Code Snippets](../VS_IDE/code-snippets.md).  
...
|--------------|-----------------|--------------|  
|`GenerateSwitchCases(` `EnumerationLiteral` `)`|Generates a switch statement and a set of case statements for the members of the enumeration specified by the `EnumerationLiteral` parameter. The `EnumerationLiteral` parameter must be either a reference to an enumeration literal or an enumeration type.|[!INCLUDE[csprcs](../VS_debugger/includes/csprcs_md.md)]|  
|`ClassName()`|Returns the name of the class that contains the inserted snippet.|[!INCLUDE[csprcs](../VS_debugger/includes/csprcs_md.md)]|  
|`SimpleTypeName(` `TypeName` `)`|Reduces the *TypeName* parameter to its simplest form in the context in which the snippet was invoked.|[!INCLUDE[csprcs](../VS_debugger/includes/csprcs_md.md)]|  
