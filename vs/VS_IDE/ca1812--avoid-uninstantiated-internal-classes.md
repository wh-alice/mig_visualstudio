...
-   Types that cannot be instantiated and that define `static` (`Shared` in Visual Basic) methods only.  
 ...
 Even though you cannot work around this limitation in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Code Analysis, the external stand-alone FxCop will occur on internal constructors if every `friend` assembly is present in the analysis.  
...
-   A private constructor for types that target [!INCLUDE[dnprdnshort](../VS_debugger/includes/dnprdnshort_md.md)] versions 1.0 and 1.1.  
  
-   The `static` (`Shared` in Visual Basic) modifier for types that target [!INCLUDE[dnprdnlong](../VS_debugger/includes/dnprdnlong_md.md)].  
...
-   The class is created automatically by the runtime or [!INCLUDE[vstecasp](../dv_TeamTestALM/includes/vstecasp_md.md)]. For example, classes that implement \<xref:System.Configuration.IConfigurationSectionHandler?displayProperty=fullName> or \<xref:System.Web.IHttpHandler?displayProperty=fullName>.  
