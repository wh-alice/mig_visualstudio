...
In [!INCLUDE[vs_current_short](../VS_debugger/includes/vs_current_short_md.md)], [!INCLUDE[ndptecclick](../VS_IDE/includes/ndptecclick_md.md)] deployment allows you to configure how an application is updated. However, if you need to use and customize advanced [!INCLUDE[ndptecclick](../VS_IDE/includes/ndptecclick_md.md)] deployment features, you will need to access the deployment object model provided by \<xref:System.Deployment.Application>. You can use the \<xref:System.Deployment.Application> APIs for advanced tasks such as:  
...
8.  Attempt to attach the debugger to the version 1 application as it downloads an update for version 2. Alternatively you can use the `System.Diagnostics.Debugger.Break` method or simply `Stop` in Visual Basic. Of course, you should not leave these method calls in production code.  
 ...
 Use the \<xref:System.Deployment.Application.ApplicationDeployment.IsNetworkDeployed*> property to invoke the \<xref:System.Deployment.Application> APIs only when the application is deployed; the APIs should not be invoked during debugging in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
