...
You can use Windows Forms to display the state of a domain-specific language (DSL) model, instead of using a DSL diagram. This topic walks you through binding a Windows Form to a DSL, using the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Visualization and Modeling SDK.  
...
2.  Experiment with the initial example that the template provides:  
 ...
 In the main instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], notice the following points about the DSL solution:  
...
```  
 ...
 Therefore we recommend that you use \<xref:Microsoft.VisualStudio.Modeling.ElementOperations> to create new elements. For more information, see [Customizing Element Creation and Movement](../VS_IDE/customizing-element-creation-and-movement.md).  
