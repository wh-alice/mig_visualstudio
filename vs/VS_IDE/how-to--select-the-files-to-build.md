...
## Specifying Inputs  
 ...
 To include files for a build, they must be included in an item list in the [!INCLUDE[vstecmsbuild](../VS_IDE/includes/vstecmsbuild_md.md)] project file. Multiple files can be added to item lists by either including the files individually or using wildcards to include many files at once.  
...
> [!NOTE]
>  You must use wildcards with items to specifiy the inputs for a build; you cannot specify the inputs using the `Sources` attribute in [!INCLUDE[vstecmsbuild](../VS_IDE/includes/vstecmsbuild_md.md)] tasks such as [Csc](../VS_IDE/csc-task.md) or [Vbc](../VS_IDE/vbc-task.md). The following example is not valid in a project file:  
>   
>  `<CSC Sources="*.cs">...</CSC>`  
