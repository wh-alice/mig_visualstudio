...
You can generate text strings in your application at run time by using [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] runtime text templates. The computer where the application executes does not have to have [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. Runtime templates are sometimes called "preprocessed text templates" because at compile time, the template generates code that is executed at run time.  
...
2.  In the **Add New Item** dialog box, select **Runtime Text Template**. (In [!INCLUDE[vbprvb](../VS_debugger/includes/vbprvb_md.md)] look under **Common Items\General**.)  
...
## Converting an Existing File to a Run-Time Template  
 A good way to create a template is to convert an existing example of the output. For example, if your application will generate HTML files, you can start by creating a plain HTML file. Make sure that it works correctly and that its appearance is correct. Then include it into your [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] project and convert it to a template.  
  
#### To convert an existing text file to a run-time template  
  
1.  Include the file into your [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] project. In Solution Explorer, on the shortcut menu of the project, choose **Add**, **Existing Item**.  
...
#### Constructor parameters in Visual Basic  
 In [!INCLUDE[vbprvb](../VS_debugger/includes/vbprvb_md.md)], the separate file **MyWebPageCode.vb** contains:  
