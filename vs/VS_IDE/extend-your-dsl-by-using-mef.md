...
9. Review and update the other fields of **source.extension.vsixmanifest**. Click **Select Editions** and verify that the correct [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] editions are set.  
...
11. To test the extension, press **F5**. In the experimental instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], create or open an example file of the DSL.  
...
### Gesture Handlers  
 A gesture handler can deal with objects dragged onto the diagram from anywhere, inside or outside [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. The following example lets the user drag files from Windows Explorer onto the diagram. It creates elements that contain the file names.  
