...
4.  Add the following methods to your class.  
    ...
    >  Permissions assigned by pre-trusting cannot exceed the permissions of the custom installer code.  
     ...
     [!code[System.Deployment.Application.InPlaceHostingManager#1](../VS_IDE/codesnippet/VisualBasic/walkthrough--creating-a-custom-installer-for-a-clickonce-application_1.vb)]
[!code[System.Deployment.Application.InPlaceHostingManager#1](../VS_IDE/codesnippet/CSharp/walkthrough--creating-a-custom-installer-for-a-clickonce-application_1.cs)]  
...
## Next Steps  
 A ClickOnce application can also add custom update logic, including a custom user interface to show during the update process. For more information, see \<xref:System.Deployment.Application.UpdateCheckInfo>. A ClickOnce application can also suppress the standard Start menu entry, shortcut, and Add or Remove Programs entry by using a `<customUX>` element. For more information, see [\<entryPoint> Element](../VS_IDE/-entrypoint--element--clickonce-application-.md) and \<xref:System.Deployment.Application.DownloadApplicationCompletedEventArgs.ShortcutAppId*>.  
