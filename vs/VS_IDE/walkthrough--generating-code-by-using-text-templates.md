...
> [!NOTE]
>  The application [xsd.exe](http://go.microsoft.com/fwlink/?LinkId=178765), which is included with [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], can generate strongly-typed classes from XML files. The template shown here is provided as an example.  
...
3.  In the file, in the template directive, change the `hostspecific` attribute to `true`.  
     ...
     This change will enable the template code to gain access to the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] services.  
...
### Accessing the Visual Studio API  
 Setting the `hostspecific` attribute of the `<#@template#>` directive allows the template to obtain access to the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] API. The template can use this to obtain the location of the project files, to avoid using an absolute file path in the template code.  
