...
Domain-Specific Language Tools (DSL Tools), which are hosted in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], let you design a domain-specific language and then generate everything that users must have to create models that are based on the language.  
...
-   DSL Library  
 ...
 The wizard creates a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] solution that has the following projects:  
...
-   **DslPackage**  
     ...
     The DslPackage project determines how the language tools integrate with [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
...
## Distributing Your DSL Solution  
 DSL Tools generates a package that is hosted in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)]. The package displays a toolbox, a DSL explorer, and other UI elements that let users create models by using your domain-specific language.  
  
 When you build and run the DSL Tools solution in [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)], a second instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] shows you how your domain-specific language looks to the user of the language. After you verify that everything works correctly, you can distribute the `.vsix` file that you will find in the build folder of the DslPackage project. This file can be used to install the DSL as a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] extension on other computers.  For more information, see [Deploying Domain-Specific Language Solutions](../VS_IDE/deploying-domain-specific-language-solutions.md).  
