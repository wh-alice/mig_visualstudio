...
-   **On opening a model.** When you open a model, validation methods can be applied to restore error messages that existed when you saved the model. Errors can also be introduced by inconsistencies between the changes that are made by users who are working on different parts of a model. For more information, see [Share models and exporting diagrams](../VS_IDE/share-models-and-exporting-diagrams.md).  
 ...
 Validation errors are reported in the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] errors window.  
...
## Installing Validation Constraints  
 Constraints are packaged within [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Extension (VSIX) files. Typically, a set of constraints will be part of an extension that also contains other definitions such as menu commands, profiles, and toolbox items.  
  
#### To install a Visual Studio Extension  
  
1.  Double-click the **.vsix** file in Windows Explorer (or File Explorer).  
  
2.  Restart any instance of [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] that is already running.  
...
1.  On the [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] **Tools** menu, click **Extensions and Updates**.  
...
3.  Restart [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)].  
