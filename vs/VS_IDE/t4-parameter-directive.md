...
In a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] text template, the `parameter` directive declares properties in your template code that are initialized from values passed in from the external context. You can set these values if you write code that invokes text transformation.  
...
## Passing parameter values to a template  
 If you are writing a [!INCLUDE[vsprvs](../dv_TeamTestALM/includes/vsprvs_md.md)] Extension such as a menu command or an event handler, you can process a template by using the text templating service:  
