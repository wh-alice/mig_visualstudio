...
|`visible`|Optional. Identifies the top-level application identity, including its dependencies. Used internally by [!INCLUDE[ndptecclick](../VS_IDE/includes/ndptecclick_md.md)] to manage application storage and activation.|  
...
## assemblyIdentity  
 Required. This element is a child of the `dependentAssembly` element. The content of `assemblyIdentity` must be the same as described in the [!INCLUDE[ndptecclick](../VS_IDE/includes/ndptecclick_md.md)] application manifest. The following table shows the attributes of the `assemblyIdentity` element.  
...
## Example  
 The following code example shows a `dependency` element in a [!INCLUDE[ndptecclick](../VS_IDE/includes/ndptecclick_md.md)] deployment manifest.  
