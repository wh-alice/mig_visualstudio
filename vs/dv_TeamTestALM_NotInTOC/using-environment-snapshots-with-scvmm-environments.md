...
> [!NOTE]
>  This is about *snapshots* of the memory and state of a [!INCLUDE[vstsLabShort](../dv_TeamTestALM/includes/vstslabshort_md.md)] environment. If you’re looking for screen snapshots that you make while testing, try [Run manual tests with Microsoft Test Manager](../dv_TeamTestALM/run-manual-tests-with-microsoft-test-manager.md).  
...
6.  If the build definition for your workflow reverts to a specific snapshot, the virtual machine cannot be locked when you try to run the tests. You must connect to the machine by using a host based connection or by using a console session before you take the snapshot to use for your workflow. For more information about this, see [How to: Connect to machines in a Lab Environment](../dv_TeamTestALM/how-to--connect-to-machines-in-a-lab-environment.md).  
 ...
 You use the [!INCLUDE[vstsLabShort](../dv_TeamTestALM/includes/vstslabshort_md.md)] **Environment Viewer** to connect to the environment and take the snapshot.  
  
#### To create a snapshot  
  
1.  In [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)], connect to the environment. In the **Lab Center**, on the **Lab** tab, select the environment, and then choose **Connect**.  
...
1.  In [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)], open the **Lab Center**, an then choose the **Lab** tab.  
...
##  <a name="SnapshotShare"></a> Sharing a Snapshot of an Environment  
 You can create a link to a snapshot of an environment that you can share with other people. The link (.lvr) file can be opened on any computer on which [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)] is installed by any user who has permissions to operate an environment in the team project.  
  
#### To create a link to a SCVMM environment snapshot  
  
1.  In [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)], in the **Lab Center**, on the **Lab** tab, make sure that the environment is **Running**.  
...
1.  In [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)], in the **Lab Center**, on the **Lab** tab, make sure that the environment is **Stopped**.  
...
1.  In [!INCLUDE[TCMext](../dv_TeamTestALM/includes/tcmext_md.md)], in the **Lab Center**, on the **Lab** tab, make sure that the environment is started.  
